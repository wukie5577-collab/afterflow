...
const phaseSteps: { phase: TrialPhase; label: string }[]=[
  { phase:'fixation', label:'fixation' },
  { phase:'adaptation', label:'adaptation' },
  { phase:'transition', label:'blank gap' },
  { phase:'motion-test', label:'motion test' },
  { phase:'response', label:'response' },
...
export function Trial({ research=false, onComplete }: { research?: boolean; onComplete?: ()=>void }) {
  ...
  const [paused,setPaused]=useState(false), [elapsed,setElapsed]=useState(0), [selected,setSelected]=useState<MotionDirection|'unsure'|null>(null), [confidence,setConfidence]=useState(3), [railOpen,setRailOpen]=useState(true)
  ...
  const duration = phase==='fixation'?1500:phase==='adaptation'?config.adaptationDurationMs:phase==='transition'?BLANK_TRANSITION_DURATION_MS:config.staticTestDurationMs
