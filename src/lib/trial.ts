...
export function validateConfig(config: TrialConfig) {
  return config.particleCount >= 24 && config.particleCount <= 1200
    ...
    && config.adaptationDotLifetimeMinMs >= 50 && config.adaptationDotLifetimeMinMs <= 2000
    && config.adaptationDotLifetimeMaxMs >= config.adaptationDotLifetimeMinMs && config.adaptationDotLifetimeMaxMs <= 5000
}
