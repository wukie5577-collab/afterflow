...
import { isStimulusPhase } from '../lib/trial'
import type { AppMode } from '../types'

const nav: [string, AppMode][] = [['EXPERIENCE','experience'],['RESEARCH','research'],['EXPLAIN','explain'],['PRESENTATION','presentation']]

export function Chrome() {
  const mode=useAppStore(s=>s.mode), phase=useAppStore(s=>s.phase), setMode=useAppStore(s=>s.setMode), muted=useAppStore(s=>s.muted), toggleMuted=useAppStore(s=>s.toggleMuted), quality=useAppStore(s=>s.quality), setQuality=useAppStore(s=>s.setQuality)
  if((mode==='research'||mode==='experience')&&isStimulusPhase(phase))return null
