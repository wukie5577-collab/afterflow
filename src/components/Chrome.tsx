...
export function Chrome() {
  ...
  if(mode==='landing')return <header className="topbar landing-topbar"><Brand onClick={()=>setMode('landing')}/><nav aria-label="Landing actions"><button onClick={()=>setMode('research')}>START EXPERIMENT</button><button onClick={()=>setMode('explain')}>HOW IT WORKS</button></nav></header>
  ...
  {(displayMode==='anaglyph'||displayMode==='cd-only')&&<aside className="stereo-calibration" aria-label="3D calibration">
    ...
    <small>{displayMode==='cd-only'?'REPEATING ONE-WAY CD: MOVE, HOLD BRIEFLY, RESET, REPEAT. X/Y, SIZE AND BRIGHTNESS STAY FIXED.':'RED-LEFT / CYAN-RIGHT GLASSES USUALLY NEED NO SWAP. ENABLE ONLY IF DEPTH IS FULLY REVERSED.'}</small>
  </aside>}</>
}
