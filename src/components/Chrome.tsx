...
export function Chrome() {
  ...
  {(displayMode==='anaglyph'||displayMode==='cd-only')&&<aside className="stereo-calibration" aria-label="3D calibration">
    ...
    <small>{displayMode==='cd-only'?'CD-ONLY APPLIES TO THE RADIAL/DEPTH CHANNEL. DOT SIZE IS CONSTANT; THE BINOCULAR MIDPOINT IS MATCHED ON EVERY DYNAMIC-RANDOM FRAME.':'RED-LEFT / CYAN-RIGHT GLASSES USUALLY NEED NO SWAP. ENABLE ONLY IF DEPTH IS FULLY REVERSED.'}</small>
  </aside>}</>
}
